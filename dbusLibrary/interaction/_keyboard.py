...
class _Keyboard(object):
	...
	def _convert_to_valid_special_key(self, key_or_text):
		if key_or_text[0:4].lower() == "KEY.".lower() :
			key = key_or_text[4:].lower()
			return key
		else:
			return key_or_text
	...
	def press_combination(self, *keys):
		...
		special_keys = []
		for longkey in keys:
			key = self._convert_to_valid_special_key(longkey)
			special_keys.append(key)
		ag.press(*special_keys)
